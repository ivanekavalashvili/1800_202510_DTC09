    ...
    db.collection("users").doc(profileUserId).get()
        .then(doc => {
            if (doc.exists) {
                ...
                // Ensure conversationId is declared before usage
                const conversationId = [currentUser.uid, profileUserId].sort().join('_');

                const contactDataForCurrentUser = {
                ...
                const contactDataForProfileUser = {
                    userId: profileUserId,
                    contactId: currentUser.uid,
                    contactName: currentUser.email || "User",
                    timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                    conversationId: conversationId
                };
                ...
                db.collection("conversations").doc(conversationId).set({
                    ...
                    .then(snapshot => {
                        if (snapshot.empty) {
                            ...
                            contactDataForCurrentUser.conversationId = conversationId;
                            return db.collection("contacts").add(contactDataForCurrentUser);
                        } else {
                    ...
                    .then(() => {
                        // Add contact for the profile user
                        return db.collection("contacts").add(contactDataForProfileUser);
                    })
                    .then(() => {
                        console.log("Contacts added successfully for both users");
                        toggleChat();
                    ...
                    .catch(error => {
                        ...
                        alert("Error adding mutual contacts. Error details: " + error.message);
                    });
            } else {
